...
# - /ui  : identificação (clara, sem “resumo/pronto”, imagens grandes)
# - /info: chat minimalista com menu (emojis) + resolver de nomes robusto
# - /predict: retorna imagem anotada e caixas
# Requer: static/pepper_info.json e (opcional) static/pimenta-logo.png
...
try:
    ...
    print("[init] Aviso: libs não prontas ainda:", e)
...
@app.get("/")
def health():
    return {"status": "ok" if READY else "warming", "ready": READY, "error": LOAD_ERR,
            "model": MODEL_PATH if MODEL_PATH else None, "classes": list(labels.values()) if READY else None}
...
def warmup():
    t0 = time.time()
    while not READY and time.time()-t0 < 90: time.sleep(0.5)
    if not READY: return {"ok": False, "warming_up": True}
    from PIL import Image
    ...
    try:
        ...
        if r.boxes is not None and len(r.boxes) > 0:
            ...
            for (x1,y1,x2,y2), c, cf in zip(xyxy, cls, conf):
                preds.append({"classe": labels.get(int(c), str(int(c))),
            ...
            return JSONResponse({"ok": True,
                                 ...
                                 "num_dets": len(preds),
                                 "top_pred": top, "preds": preds,
                                 "image_b64": image_b64, "image_url": image_url})
        return JSONResponse({"ok": True, "inference_time_s": round(time.time()-t0,3),
...
btnCam.onclick = async ()=>{
  try{
    ...
    inputCamera.value = ""; inputCamera.click(); // fallback: seletor/câmera nativa
  }
};
...
document.getElementById('btnSend').onclick = async ()=>{
  ...
  try{
    ...
    if(d.top_pred && d.top_pred.classe){
      ...
      chatBtn.style.display = "none"; lastClass = null; alert("Nenhuma pimenta detectada.");
    }
  }catch(e){ alert("Falha ao chamar a API."); }
...
<link rel="icon" href="/static/pimenta-logo.png" type="image/png" sizes="any">
<style>
:root{ --bg:#eae6df; --bubble:#fff; --me:#dcf8c6; --text:#111; --top:#075E54; }
*{box-sizing:border-box}
...
let currentPepper = (qs.get("pepper")||"").trim();
let KB = null;
...
function normalize(s){
  return (s||"").toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g,'') // remove acentos (jalapeño -> jalapeno)
...
function keyFromAny(raw){
  ...
  for(const k of Object.keys(KB)){
    const n = normalize(KB[k].nome).replace(/-?pepper$/,'').replace(/-/g,' ').trim();
    if(n===want) return k;
  }
  // 3) sinônimos exatos
  for(const k of Object.keys(KB)){
    const syns = (KB[k].sinonimos||[]).map(s=>normalize(s));
...
document.getElementById('send').onclick = async ()=>{
  const raw = document.getElementById('txt').value.trim(); if(!raw) return;
  ...
  // nome/sinônimo digitado
  const maybe = keyFromAny(raw);
  if(maybe){
    currentPepper = maybe;
    setHeader();
  ...
  if(/^\d+$/.test(raw)){
    ...
    if(out==="Opção inválida.") addMsg("❌ Opção inválida.\n\n"+menuText());
    else { addMsg(out); addMsg("—\n"+menuText()); }
    return;
  }
...
(async function init(){
  ...
  // tenta resolver a pimenta vinda da detecção (?pepper=...)
  if(currentPepper){
    const k = keyFromAny(currentPepper);
    if(k) currentPepper = k;
  }
  if(!currentPepper || !KB[currentPepper]) currentPepper = Object.keys(KB)[0];

  setHeader();
  addMsg("Olá! 👋 Bem-vindo ao *Chat sobre Pimentas*.");
  addMsg(menuText());
})();
</script>
</body></html>
